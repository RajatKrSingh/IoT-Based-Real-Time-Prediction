# -*- coding: utf-8 -*-
"""
Created on Fri Jul 07 11:19:41 2017
...
def probcal(z_hmm,len1,len2):
    z_input = np.array([])
    c_arr = np.zeros(shape=(27,1))
    check_noise = np.loadtxt('output_fresh.txt')
...
def supervised_learning():
    ...
    slope_y = np.array([])
    slope_z = np.array([])
    input_data = np.loadtxt('fresh_no_noise.txt')

    datax = pd.rolling_mean(input_data[:,0],5)
    datay = pd.rolling_mean(input_data[:,1],5) 
    dataz = pd.rolling_mean(input_data[:,2],5)
    
    datax = np.vstack((datax,datay,dataz))
    input_data = np.transpose(datax)
    #input_data = input_data.astype(int)
    #np.savetxt('no_noise.txt',input_data,fmt='%d')
    output_label = np.array([])
    k1,k2 =  input_data.shape
    print k1,k2
    for i in np.arange(0,k1-1):
    ...
    len_pass1 = slope_x.size
    tempp = np.loadtxt('fresh_noise.txt')
    
    datax = pd.rolling_mean(tempp[:,0],5)
    datay = pd.rolling_mean(tempp[:,1],5) 
    dataz = pd.rolling_mean(tempp[:,2],5)    
    datax = np.vstack((datax,datay,dataz))
    tempp = np.transpose(datax)
    #tempp = tempp.astype(int)
    k1,k2 = tempp.shape
    for i in np.arange(0,k1-1):
        slope_x = np.append(slope_x,tempp[i+1][0]-tempp[i][0])
        slope_y = np.append(slope_y,tempp[i+1][1]-tempp[i][1])
        slope_z = np.append(slope_z,tempp[i+1][2]-tempp[i][2])
    for i in np.arange(0,k1):
        output_label = np.append(output_label,1)
    len_pass2,g = tempp.shape
    len_pass2 = len_pass2 -1
...
def makeFile_func():
    ...
    execution_str = "SELECT sensor_id,receive_time,data_received FROM SENSOR_DATA order by receive_time desc limit 10000"    
    ...
    data_s1 = pd.rolling_mean(data_s1,5)
    data_s2 = pd.rolling_mean(data_s2,5)
    data_s3 = pd.rolling_mean(data_s3,5)
    plt.plot(data_s1)
    ...
    slope_y = np.array([])
    slope_z = np.array([])
    for i in np.arange(1,data_s1.size-1):
        slope_x = np.append(slope_x,data_s1[i+1] - data_s1[i])
        slope_y = np.append(slope_y,data_s2[i+1] - data_s2[i])
        slope_z = np.append(slope_z,data_s3[i+1] - data_s3[i] )   
    slope_x = data_s1
    slope_y = data_s2
    slope_z = data_s3
    ...
    for i in np.arange(0,count_array_x.size):
        if count_array_x[i] >9:
            count_array_x[i] = 20
        elif count_array_x[i] >=4:
            count_array_x[i] = 10
        elif count_array_x[i]>=0:
            count_array_x[i] = 5

        if count_array_y[i] >9:
            count_array_y[i] = 20
        elif count_array_y[i] >=5:
            count_array_y[i] = 10
        elif count_array_y[i]>=0:
            count_array_y[i] = 5
            
        if count_array_z[i] >9:
            count_array_z[i] = 20
        elif count_array_z[i] >=5:
            count_array_z[i] = 10
        elif count_array_z[i]>=0:
            count_array_z[i] = 5
    
    np.savetxt('freshcountp.txt',count_array_x)
#-------------------------------------------------------------------------------------------------------------
# FEATURE 3
#-------------------------------------------------------------------------------------------------------------    
...
#------------------------------------------------------------------------------------------------------------
    ...
    np.savetxt('fresh_prediction_data.txt',to_save)
    db.close   
...
def changes(count_array,top):
    ...
    if(top>20):
        start = top - 20
    curr_val = count_array[top]
    while((start<top) and ((curr_val==0) or (curr_val==1))):
        if(count_array[top] == curr_val):
            retval = retval + 1
            top = top - 1
        else:
            break
...
def tryHmm(observed,input_data,hidden,len1,len2):
    ...
    for i in np.arange(0,count_array_x.size):
        ...
        if count_array_z[i] >9:
            ...
            count_array_z[i] = 5 
    np.savetxt('fresh_count.txt',count_array_x)
#-------------------------------------------------------------------------------------------------------------
# FEATURE 3
#-------------------------------------------------------------------------------------------------------------    
    feature3_x =np.array([])
...
#-------------------------------------------------------------------------------------------------------------
    ...
    np.savetxt('xinput2.txt',x_input)
    c_arr = c_arr+c_arr1
    print x_input.size
    ...
    np.savetxt('emission.txt',emission_probability)
...
# HMM MODELS ACCROSS VARIOUS AXIS
  #--------------------------------------------------------------------------------------------------------------
    ...
    while 1<2:
        makeFile_func()
        time.sleep(0.1)
        prediction_file = np.loadtxt('fresh_prediction_data.txt')
        ...
        check_noise = np.loadtxt('timex_output.txt',dtype='str')
        rw = check_noise.size
        print 'row',rw
        for i in np.arange(0,rw):
            k = datetime.datetime.strptime(check_noise[i],"%H:%M:%S.%f").time()
            count = 0
            ul = rw-1-i
            if rw>20:
                ul = ul - 20
            for j in np.arange(0,ul):
                val = datetime.datetime.strptime(check_noise[i+j],"%H:%M:%S.%f").time()
                todelta = datetime.datetime.combine(datetime.date.min,val) - datetime.datetime.combine(datetime.date.min,k)
                if todelta < datetime.timedelta(0,4):
                    count = count + 1
                else:
                    break
                print 'this is',count
            if count>=10:
                final_out = np.loadtxt('final_out.txt',dtype='str')
                if str(k) not in final_out:
                    final_out = np.append(final_out,k)
                final_out = np.sort(final_out,axis=0)
                #final_out = np.unique(final_out)
                np.savetxt('final_out.txt',final_out,fmt= '%s')
